  ...
  return (
    <section className="mt-20 bg-gray-100 dark:bg-gray-900">
      <div className="text-center">
        <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-gray-900 dark:text-white">What You'll Find</h2>
        <p className="mt-3 text-slate-600 dark:text-gray-300">Everything you need to build in public and grow with peers.</p>
      </div>
      <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {features.map((f, i) => (
          <motion.div
            ...
            className="group rounded-2xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-sm hover:shadow-md transition-shadow"
          >
            <div className="flex items-center gap-4">
              <span className="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-blue-50 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400">
                <f.icon className="h-6 w-6" />
              </span>
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{f.title}</h3>
            </div>
            <p className="mt-3 text-slate-600 dark:text-gray-300">{f.desc}</p>
          </motion.div>
        ))}
      </div>
    </section>
